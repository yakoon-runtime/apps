...
def _platform_mgr(root: Path) -> InstallationManager:
    repos = root / "repos"
...
def test_update_heals_deleted_platform_component():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        mgr = _platform_mgr(root)
        inst = root / "inst"
        ...
        boot = inst / ".yak" / "components" / "boot"
        assert boot.exists()
        shutil.rmtree(boot)

        mgr.update(inst)
        assert (inst / ".yak" / "components" / "boot" / "structure").exists()
...
def test_mode_switch_replaces_component(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        repos = root / "repos"
        (repos / "y5n-packs-erp" / "structure").mkdir(parents=True)
        (repos / "y5n-packs-erp" / "pack.toml").write_text(
            'name = "erp"\nversion = "0.1"\nmount = "/opt/erp"\n'
        )
        mgr.update(inst)
        ...
        shutil.rmtree(repos / "y5n-packs-erp")
        mgr.update(inst)
...
def test_update_removes_orphan_components():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        mgr = _platform_mgr(root)
        inst = root / "inst"
...
def test_add_rolls_back_partial_staging(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(RuntimeError, match="boom"):
            mgr.add("system", inst)
        assert not (inst / ".yak" / "components" / "system").exists()
