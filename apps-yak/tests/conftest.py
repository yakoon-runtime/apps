...
def make_source(
    ...
    """Create a source directory with a declared catalog.yml (ADR-23 Step 4).

    The catalog is a ``name → location`` mapping — the key is a discovery
    binding only, never a normative identity. Each location must be a
    component root, so a ``.yak/component.yml`` declaring the component's
    own name is created at every listed location (never overwritten).
    """
    ...
    if components:
        for name, entry in components.items():
            location = entry if isinstance(entry, str) else entry.get("location")
            lines.append(f"  {name}:")
